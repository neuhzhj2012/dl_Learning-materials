...
> 
> 即：输出层的每个位置预测的box与GT有交集时为正样本，否则为负样本。不过**预测值与G的IOU值是在输出层大小上进行，而不是在原图大小上计算的**。
...
> FCOS使用FPN的不同层分别预测不同大小的物体(大物体出现在较高语义的特征图上，同时也会出现在低层语义的特征图，而小物体只出现在低层语义的特征图上。所以通过对不同层限定了物体的大小来分别预测)。因为重叠物体的尺度变化非常大，基于FPN的多尺度检测可以在不同层检测不同大小的物体，从而减少大物体包含小物体的情况。
